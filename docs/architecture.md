...
- `packages/studio` is a private desktop application workspace distributed through native installers, not npm.
...
packages/
  ...
  studio/              private standalone desktop client
```
...
6. The GUI ships as a standalone desktop release with a bundled CLI and an allow-listed IPC boundary.
